...
export const createFiles = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        for (const file of args.files) {
            ...
            if (existing) {
                results.push({ name: file.name, error: "File with the same name already exists in this folder" });
                continue;
            }
...
export const createFolder = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        if (existing) {
            throw new Error("Folder with the same name already exists in this folder");
        }
...
export const renameFile = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        const existing = siblings.find((f) => f.name === args.newName && f.type === file.type && f._id !== args.fileId);

        if (existing) {
            throw new Error(`A ${file.type} with the same name already exists in this folder`);
        }

        await ctx.db.patch(args.fileId, { name: args.newName, updatedAt: Date.now() });
...
export const deleteFile = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        // Recursively delete file/folder and its children
