...
export const createReadFilesTool = ({ internalKey }: ReadFilesToolOptions) => {
    return createTool({
        name: "read-files",
        description: "Reads the content of files from the project. Returns file contents.",
        parameters: z.object({
            fileIds: z.array(z.string()).describe("Array of file IDs to read the content from."),
        }),
        handler: async ( params, { step: toolStep } ) => {
            ...
            try {
                return await toolStep?.run("read-files", async () => {
                    ...
                    if (results.length === 0) {
                        return "No files found or files are empty.";
                    }
